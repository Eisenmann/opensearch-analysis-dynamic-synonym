...
/**
 * Create by guanquan.wang at 2019-09-18 16:55, edited by eisenmann at 2023-05-19 12-00
 */
public class DynamicSynonymPluginTest {
    ...
    public void setUp() {
        ...
        runner.build(newConfigs()
                ...
                .pluginTypes("com.eisenmann.opensearch.plugin.DynamicSynonymPlugin")
        );
    }
    ...
    private synchronized void analyzer(String indexName) throws InterruptedException {
        List<AnalyzeAction.AnalyzeToken> tokens = tokens(indexName, "делать");
        ...
        tokens = tokens(indexName, "делать");
        for (AnalyzeAction.AnalyzeToken token : tokens) {
            System.out.println(token.getTerm() + " => " + token.getType());
        }

        tokens = tokens(indexName, "глупо");
        for (AnalyzeAction.AnalyzeToken token : tokens) {
            System.out.println(token.getTerm() + " => " + token.getType());
        }
    }
